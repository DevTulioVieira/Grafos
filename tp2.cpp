...
void menu(){
    ...
    cout<<"12.Editar nome dos no"<<endl;
...
void carregarGrafo(noLista *lista, int *numero){
    ...
    *numero=direcionado;
    ...
    for(total=0;total!=i;total++){    
        inFile>>novo->id>>novo->eixox>>novo->eixoy>>novo->nome;
        novo->total=0;
        novo->verificado=0;
        novo->primeiro = NULL;
...
void mostrarNO(noLista *lista){
    ...
    if(aux==NULL){
        ...
        while(aux!= NULL){
            cout<<"NO:  "<<aux->id;
            cout<<"   Coordenada("<<aux->eixox<<","<<aux->eixoy<<") Nome: "<<aux->nome<<endl;
            aux = aux->prox;
        }
    }
}
...
void editarnome(noLista *lista){
    ...
    if(aux==NULL){
        ...
        cout<<"Informe o ID do no que deseja alterar o nome: ";
        cin>>num;
        while(aux!= NULL){
            if(num==aux->id){
                cout<<"Nome atual: "<<aux->nome<<"\nInsira o novo nome: ";
                cin.ignore();
                cin>>aux->nome;
        ...
        cout<<"Id n localizado!"<<endl;
    }
}

void resetar(noLista *lista){
    no *aux;
...
void buscaPro(no *aux, no *ini){
    liga *correndo;
    no *ajuda;

    correndo=aux->primeiro;

    if(aux->verificado==1){
        cout<<"No "<<aux->id<<" ja encontrado!"<<endl;
        return;
    }else{
        aux->verificado=1;
        while(correndo!=NULL){
            cout<<"Verificando ligacao com "<<correndo->ponto<<endl;
            ajuda=ini;
            while(ajuda!=NULL){
                if(ajuda->id==correndo->ponto && ajuda->verificado==0){
                    cout<<"Ligacao encontrada com no: "<<ajuda->id<<endl;
                    buscaPro(&*ajuda, &*ini);
                }
                ajuda=ajuda->prox;
            }
            correndo=correndo->pro;
        }
        cout<<"No: "<<aux->id<<" finalizado!"<<endl;
    }
}

void buscaProfundidade(noLista *lista){
    no *aux, *ini;
    ...
    if(aux==NULL){
        ...
        cout<<"Comecando busca em profundidade!"<<endl;
        while(aux!=NULL){
            cout<<"NO sendo analisado: "<<aux->id<<endl;
            buscaPro(&*aux, &*ini);
            aux=aux->prox;
        }
    }
...
void busLargura(no *aux, int vet1[], int *len){
    liga *ajuda;
...
void buscarLargura(noLista *lista){
    no *aux, *help;
    liga *ajuda;
    aux=lista->inicio;
    ...
    int vet1[20];
    int vet2[20];
    int i, j, k, len2=1, len1=1, continua=0, raiz=0;

    if(aux==NULL){
        cout<<"procurando enquenca!"<<endl;
    }else{
        vet2[0]=aux->id;
        while(continua==0){//
        continua=1;
        ...
        len2=len1;
        len1=0;
        cout<<"\nVerificando No de nivel: "<<raiz<<endl;
        raiz+=1;
        for(i=0; i<len2; i++){
            help=lista->inicio;
            cout<<"Veificando NO: "<<vet2[i];
            while(help!=NULL){
                if(help->id==vet2[i]){
                    ajuda=help->primeiro;
                    //
                    if(help->verificado==0){
                        help->verificado=1;
                        continua=0;
                        cout<<"\nLigado com NO: ";
                        while(ajuda!=NULL){
